...
.UUri Mapping
[[uri-mapping]]
[cols="1a,1a,3a", width="100%", options="header"]
|===
...
[[uattribute-mapping]]
[cols=".^m,.^m,.^m,.^m,.^m,.^m,.^m,.^m", width="100%", options="autowidth"]
|===
.2+^.^h| UMessageType .2+^.^h| SOME/IP Type 2+^.^h| Message ID 2+^.^h| Request ID .2+^.^h| Interface Version .2+^.^h| Return Code (commstatus)
^.^h| ServiceID ^.^h| MethodID ^.^h| ClientID ^.^h| SessionID 

| NOTIFICATION  | REQUEST_NO_RETURN (0x01) m| sink.ue_id link:#note-1[^(1)^]   | sink.resource_id   | source.ue_id=0 | link:#note-2[^(2)^],link:#note-3[^(3)^] | sink.ue_major_version   | 
 ...
 * `Client ID` for vsomeip transport *SHOULD* be set via configuration to `source.ue_id`. It *MUST* be unique for the network.
 * `REQUEST_NO_RETURN` is a Request in SOME/IP that applies to `Method ID`, but in uProtocol `NOTIFICATION` applies for `Event ID`.
