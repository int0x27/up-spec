...
|===
m| UUri Portion m| SOME/IP Field a| Description

m| authority_name m| IP address & port
...
m| ue_id m| Instance ID \| Service ID
...
m| ue_version_major m| Interface Version
...
m| resource_id m| Method ID / Event ID a| Identifier of the method/event placed in the
